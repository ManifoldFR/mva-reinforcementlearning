...
The value update step in \cref{algo:AdvantageActorCritic} (and also \cref{algo:BaseActorCritic}) can be seen as a supervised regression step, where we fit the (bootstrapped) returns ${\redfont G_{t:t+1}} = r_t + \widehat{v}_\nu(s_{t+1})$ to the value model predictions $\widehat{v}_\nu(s_t)$, with a mean-squared loss
\[
	\calL(\nu) =
